    ...
    meta_parse_ctx_t *ctx)
{
    ...
    const char *type_name = token->type;
    ecs_entity_t type = 0;

    /* Parse vector type */
    if (!token->is_ptr) {
        if (!ecs_os_strcmp(type_name, "ecs_array")) {
            type = meta_lookup_array(world, 0, token->params, ctx);

        } else if (!ecs_os_strcmp(type_name, "ecs_vector") || 
                !ecs_os_strcmp(type_name, "flecs::vector")) 
        {
            type = meta_lookup_vector(world, 0, token->params, ctx);

        } else if (!ecs_os_strcmp(type_name, "flecs::bitmask")) {
            type = meta_lookup_bitmask(world, 0, token->params, ctx);

        } else if (!ecs_os_strcmp(type_name, "flecs::byte")) {
            type = ecs_id(ecs_byte_t);

        } else if (!ecs_os_strcmp(type_name, "char")) {
            type = ecs_id(ecs_char_t);

        } else if (!ecs_os_strcmp(type_name, "bool") || 
                !ecs_os_strcmp(type_name, "_Bool")) 
        {
            type = ecs_id(ecs_bool_t);

        } else if (!ecs_os_strcmp(type_name, "int8_t")) {
            type = ecs_id(ecs_i8_t);
        } else if (!ecs_os_strcmp(type_name, "int16_t")) {
            type = ecs_id(ecs_i16_t);
        } else if (!ecs_os_strcmp(type_name, "int32_t")) {
            type = ecs_id(ecs_i32_t);
        } else if (!ecs_os_strcmp(type_name, "int64_t")) {
            type = ecs_id(ecs_i64_t);

        } else if (!ecs_os_strcmp(type_name, "uint8_t")) {
            type = ecs_id(ecs_u8_t);
        } else if (!ecs_os_strcmp(type_name, "uint16_t")) {
            type = ecs_id(ecs_u16_t);
        } else if (!ecs_os_strcmp(type_name, "uint32_t")) {
            type = ecs_id(ecs_u32_t);
        } else if (!ecs_os_strcmp(type_name, "uint64_t")) {
            type = ecs_id(ecs_u64_t);

        } else if (!ecs_os_strcmp(type_name, "float")) {
            type = ecs_id(ecs_f32_t);
        } else if (!ecs_os_strcmp(type_name, "double")) {
            type = ecs_id(ecs_f64_t);

        } else if (!ecs_os_strcmp(type_name, "ecs_entity_t")) {
            type = ecs_id(ecs_entity_t);

        } else if (!ecs_os_strcmp(type_name, "ecs_id_t")) {
            type = ecs_id(ecs_id_t);

        } else if (!ecs_os_strcmp(type_name, "char*")) {
            type = ecs_id(ecs_string_t);
        } else {
            type = ecs_lookup_symbol(world, type_name, true, true);
        }
    } else {
        if (!ecs_os_strcmp(type_name, "char")) {
            type_name = "flecs.meta.string";
        } else
        if (token->is_ptr) {
            type_name = "flecs.meta.uptr";
        } else
        if (!ecs_os_strcmp(type_name, "char*") || 
            !ecs_os_strcmp(type_name, "flecs::string")) 
        {
            type_name = "flecs.meta.string";
        }
    ...
    if (!type) {
        ecs_meta_error(ctx, ptr, "unknown type '%s'", type_name);
        goto error;
    }
