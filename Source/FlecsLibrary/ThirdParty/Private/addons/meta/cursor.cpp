    ...
    ecs_meta_scope_t *scope)
{
    ...
    if (scope->vector) {
        ...
        if (scope->is_collection) {
            ...
            return static_cast<ecs_meta_type_op_t*>(opaque_ptr);
        } else if (op->name) {
            ...
            return static_cast<ecs_meta_type_op_t*>(opaque->ensure_member(scope->ptr, op->name));
        } else {
    ...
    return static_cast<ecs_meta_type_op_t*>(ECS_OFFSET(scope->ptr, size * scope->elem_cur + op->offset));
}
    ...
    ecs_meta_cursor_t *cursor)
{
    ...
    /* Vector push */
    case EcsOpVector: {
        next_scope->vector = static_cast<ecs_vec_t*>(ptr);
    ...
    const char *value)
{
    ...
    #ifdef FLECS_SCRIPT
        ecs_id_t id = 0;
        if (flecs_id_parse(cursor->world, nullptr, value, &id) == nullptr) {
            goto error;
        }
    ...
    const ecs_meta_cursor_t *cursor)
{
    ...
    case EcsOpString: return *(const char**)ptr != nullptr;
