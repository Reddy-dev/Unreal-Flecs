...
BEGIN_DEFINE_SPEC(FCollectionSparseFlecsSpec,
                                  "FlecsLibrary.Collections.Sparse",
...
END_DEFINE_SPEC(FCollectionSparseFlecsSpec);

void FCollectionSparseFlecsSpec::Define()
{
    Describe("Sparse Creation and Deletion", [this]()
    {
        It("Should create and delete an element in a sparse array", [this]()
        {
            Sparse_create_delete();
        });

        It("Should create and delete two elements in a sparse array", [this]()
        {
            Sparse_create_delete_2();
        });
    });
    
    Describe("Sparse Addition and Removal", [this]()
    {
        It("Should add a single element to a sparse array", [this]()
        {
            Sparse_add_1();
        });

        It("Should add a single element to an empty sparse array", [this]()
        {
            Sparse_add_1_to_empty();
        });

        It("Should add a single element to a sparse array with chunk size 1", [this]()
        {
            Sparse_add_1_chunk_size_1();
        });

        It("Should add multiple elements to a sparse array", [this]()
        {
            Sparse_add_n();
        });

        It("Should add multiple elements to a sparse array with chunk size 1", [this]()
        {
            Sparse_add_n_chunk_size_1();
        });
    });

    Describe("Sparse Removal", [this]()
    {
        It("Should remove an element from a sparse array", [this]()
        {
            Sparse_remove();
        });

        It("Should remove the first element from a sparse array", [this]()
        {
            Sparse_remove_first();
        });

        It("Should remove the last element from a sparse array", [this]()
        {
            Sparse_remove_last();
        });

        It("Should remove all elements from a sparse array", [this]()
        {
            Sparse_remove_all();
        });

        It("Should remove all elements from a sparse array with multiple chunks", [this]()
        {
            Sparse_remove_all_n_chunks();
        });
    });

    Describe("Sparse Clearing", [this]()
    {
        It("Should clear a sparse array with one element", [this]()
        {
            Sparse_clear_1();
        });

        It("Should clear an empty sparse array", [this]()
        {
            Sparse_clear_empty();
        });

        It("Should clear a sparse array with multiple elements", [this]()
        {
            Sparse_clear_n();
        });

        It("Should clear a sparse array with multiple chunks", [this]()
        {
            Sparse_clear_n_chunks();
        });
    });

    Describe("Sparse Addition after Clearing", [this]()
    {
        It("Should add an element to a sparse array after clearing", [this]()
        {
            Sparse_add_after_clear();
        });
    });

    Describe("Sparse Count of Null", [this]()
    {
        It("Should return 0 for count of null sparse array", [this]()
        {
            Sparse_count_of_null();
        });
    });

    Describe("Sparse Ensure", [this]()
    {
        It("Should return null for try after ensure high", [this]()
        {
            Sparse_try_low_after_ensure_high();
        });

        It("Should return false for is alive after ensure high", [this]()
        {
            Sparse_is_alive_low_after_ensure_high();
        });

        It("Should not remove low after ensure high", [this]()
        {
            Sparse_remove_low_after_ensure_high();
        });
        
        It("Should skip generation when ensuring", [this]()
        {
