...
BEGIN_DEFINE_SPEC(FMapFlecsTestsSpec,
                  "FlecsLibrary.Collections.Map",
...
END_DEFINE_SPEC(FMapFlecsTestsSpec);

void FMapFlecsTestsSpec::Define()
{
	Describe("Map Count", [this]()
    {
        It("Should return the correct count", [this]()
        {
            Map_count();
        });

	    It("Should return the correct count for empty map", [this]()
        {
            Map_count_empty();
        });
    });

    Describe("Map Set/Get", [this]()
    {
        It("Should overwrite existing value", [this]()
        {
            Map_set_overwrite();
        });

        It("Should rehash when inserting new values", [this]()
        {
            Map_set_rehash();
        });

        It("Should get existing value", [this]()
        {
            Map_get();
        });

        It("Should get all values", [this]()
        {
            Map_get_all();
        });

        It("Should return null for empty map", [this]()
        {
            Map_get_empty();
        });

        It("Should return null for unknown key", [this]()
        {
            Map_get_unknown();
        });

        It("Should return null for 0 key in empty map", [this]()
        {
            Map_get_0_from_empty();
        });

        It("Should return null for 0 key in populated map", [this]()
        {
            Map_get_0_from_populated();
        });

        It("Should return value for 0 key after insert", [this]()
        {
            Map_get_0_after_insert();
        });

        It("Should return value for 0 key after ensure", [this]()
        {
            Map_get_0_after_ensure();
        });
    });

    Describe("Map Remove", [this]()
    {
        It("Should remove existing value", [this]()
        {
            Map_remove();
        });

        It("Should do nothing for empty map", [this]()
        {
            Map_remove_empty();
        });

        It("Should do nothing for unknown key", [this]()
        {
            Map_remove_unknown();
        });

        It("Should do nothing for unknown key twice", [this]()
        {
            Map_remove_twice();
        });
    });

    Describe("Map Clear", [this]()
    {
        It("Should do nothing for empty map", [this]()
        {
            Map_clear_empty();
        ...
        It("Should clear populated map", [this]()
        {
            Map_clear_populated();
        });

        It("Should do nothing for empty map twice", [this]()
        {
            Map_clear_empty_twice();
        });

        It("Should clear populated map twice", [this]()
        {
            Map_clear_populated_twice();
        });

        It("Should populate after clear", [this]()
        {
            Map_populate_after_clear();
        });
    });

    Describe("Map Iteration", [this]()
    {
        It("Should iterate over all values", [this]()
        {
            Map_iter();
        });

        It("Should not iterate over empty map", [this]()
        {
            Map_iter_empty();
        });

        It("Should not iterate over null map", [this]()
        {
            Map_iter_null();
        });
    });

    Describe("Map Randomized", [this]()
    {
        It("Should insert randomized values", [this]()
        {
            Map_randomized_insert();
        });

        It("Should remove randomized values", [this]()
        {
            Map_randomized_remove();
        });

        It("Should insert large randomized values", [this]()
        {
            Map_randomized_insert_large();
        });

        It("Should remove large randomized values", [this]()
        {
            Map_randomized_remove_large();
        });

        It("Should insert randomized values after clear", [this]()
        {
