...
#include "FlecsCPPTestTypes.h"
#include "Bake/FlecsTestUtils.h"

BEGIN_DEFINE_SPEC(FIterableCPPFlecsTestsSpec,
                  "FlecsLibrary.CPP.Iterable",
...
void Iterable_page_each(void) {
    auto e1 = World->entity(); e1.set<flecs_cpp_tests::Self>({e1});
    auto e2 = World->entity(); e2.set<flecs_cpp_tests::Self>({e2});
    auto e3 = World->entity(); e3.set<flecs_cpp_tests::Self>({e3});
    auto e4 = World->entity(); e4.set<flecs_cpp_tests::Self>({e4});
    auto e5 = World->entity(); e5.set<flecs_cpp_tests::Self>({e5});

    auto q = World->query<flecs_cpp_tests::Self>();

    int32_t count = 0;
    q.page(1, 3).each([&](flecs::entity e, flecs_cpp_tests::Self& self) {
        count ++;
        test_assert(e != e1);
        test_assert(e != e5);
        test_assert(e == self.value);
...
void Iterable_page_iter(void) {
    ...
    q.page(1, 3).run([&](flecs::iter it) {
        while (it.next()) {
            auto self = it.field<flecs_cpp_tests::Self>(0);
            test_int(it.count(), 3);
            test_assert(it.entity(0) == e2);
            test_assert(it.entity(1) == e3);
            test_assert(it.entity(2) == e4);
...
void Iterable_worker_each(void) {
    ...
    int32_t count = 0;
    q.worker(0, 2).each([&](flecs::entity e, flecs_cpp_tests::Self& self) {
        count ++;
        test_assert(e != e4);
        test_assert(e != e5);
        test_assert(e == self.value);
    ...
    count = 0;
    q.worker(1, 2).each([&](flecs::entity e, flecs_cpp_tests::Self& self) {
        count ++;
        test_assert(e != e1);
        test_assert(e != e2);
        test_assert(e != e3);
...
END_DEFINE_SPEC(FIterableCPPFlecsTestsSpec);

void FIterableCPPFlecsTestsSpec::Define()
{
	BeforeEach([&]()
	{
		World = std::make_unique<flecs::world>();
	    World->component<flecs_cpp_tests::Self>();
	});
