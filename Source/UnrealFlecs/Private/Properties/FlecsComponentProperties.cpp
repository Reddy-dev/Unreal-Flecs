...
namespace UE::Flecs::Private
{
	...
	void AddRegisteredComponentProperties_Static(const FFlecsComponentPropertiesDefinition& InDefinition)
	{
		...
		solid_cassumef(!bAlreadyPending, TEXT("Component with name %s is already pending registration!"), *InDefinition.Name);
		...
		const FString& InModuleName, const FString& InPluginName)
	{
		...
		else if (InPluginName.IsEmpty() == false)
		{
			const FFlecsModuleRegistryRegisteredItem* PluginRegisteredItem
				= FFlecsModuleRegistry::Get().FindRegisteredPlugin(FName(InPluginName));
			
			if (PluginRegisteredItem)
			{
